...
class ColorInput < SimpleForm::Inputs::CollectionSelectInput
  def input(_wrapper_options)
    ...
    collection.each do |color|
      ...
      menu_item = ActiveSupport::SafeBuffer.new
      menu_item = content_tag(:div, collection_item, class: "ui item #{'active' if color == object.send(attribute_name)}", data: {value: color})
    ...
    #menu = content_tag(:div, menu_items, class: 'menu').html_safe
